...
template = pd.read_csv("chris2/template.csv")
#template = pd.read_csv("chris_2smoothtemp2.csv")
#template = pd.read_csv("chris_2smoothtemp2_rn.csv")
...
Features["Wv_lbounds"] = Features.Wv_lbounds.values
Features["Wv_ubounds"] = Features.Wv_ubounds.values
Features["MinWvl"] = Features.MinWvl.values
...
import glob
filenames = [f for f in glob.glob("chris2/chrissim_*")]
filenames = np.array(filenames)[np.argsort([float(f.split("chrissim_")[1].split(".csv")[0]) for f in filenames])]
...
pvals.to_csv("timecorssim_SAFE.csv")
