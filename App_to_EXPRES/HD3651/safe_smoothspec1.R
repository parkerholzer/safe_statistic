library(parallel)
filenames = list.files(path = "3651spec", pattern = "*ctd.csv")
options(digits=10)
srtid = as.numeric(gsub("ctd.csv", "", gsub("3651_", "", filenames)))
filenames = filenames[order(srtid)]
vels = read.csv("3651spec/3651.txt")
...
SPECTRA = mclapply(filenames, function(f) read.csv(paste0("3651spec/",f)), mc.cores=19)
...
write.csv(smoothtemp, file = "3651smoothtemp1.csv", row.names=FALSE)
