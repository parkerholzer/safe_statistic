...
template = pd.read_csv("3651smoothtemp1.csv")
...
import glob
filenames = [f for f in glob.glob("3651spec/*ctd.csv")]
filenames = np.array(filenames)[np.argsort([float(f.split("3651_")[1].split("ctd")[0]) for f in filenames])]
#filenames = np.array(filenames)[np.argsort([float(f.split(".csv")[0].split("chris/")[1]) for f in filenames])]
vels = pd.read_csv("3651spec/3651.txt")
