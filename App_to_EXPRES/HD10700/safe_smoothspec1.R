library(parallel)
filenames = list.files(path = "10700spec", pattern = "*ctd.csv")
options(digits=10)
srtid = as.numeric(gsub("ctd.csv", "", gsub("10700_", "", filenames)))
filenames = filenames[order(srtid)]
vels = read.csv("10700spec/10700.txt")
...
SPECTRA = mclapply(filenames, function(f) read.csv(paste0("10700spec/",f)), mc.cores=19)
...
write.csv(smoothtemp, file = "10700smoothtemp1.csv", row.names=FALSE)
